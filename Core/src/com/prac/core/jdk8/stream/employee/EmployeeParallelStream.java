...
public class EmployeeParallelStream {
	...
	public static void main(String[] args) {
		int noOfRecords = 10000; // How many records you want
...
//		empDistinct.forEach(System.out::println);
		...
		Map<Object, Object> dept = empDistinct.stream().collect(Collectors.groupingBy(Employee::getDepartment))
				.entrySet().stream().collect(Collectors.toMap(x -> x, y -> y.getValue().stream().count()));
		long start;
		long end;
		start = Calendar.getInstance().getTimeInMillis();

		Map<DepartmentCode, List<Employee>> dept1 = empDistinct.stream()
...
//start = Calendar.getInstance().getTimeInMillis();
		...
		Map<DepartmentCode, List<Employee>> dept2 = empDistinct.parallelStream()
				.collect(Collectors.groupingBy(Employee::getDepartment));
//end = Calendar.getInstance().getTimeInMillis();
		end = ZonedDateTime.now().toInstant().toEpochMilli();
		System.out
				.println("For " + noOfRecords + " records Parallel Stream Took: " + (end - start) + " MilliSeconds\n");
